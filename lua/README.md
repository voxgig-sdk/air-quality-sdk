...
The Lua SDK for the AirQuality API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:AirQuality()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/air-quality-sdk/releases)),
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local air_quality, err = client:AirQuality():load()
    if err then error(err) end
...
```lua
local air_quality, err = client:AirQuality():load({ latitude = 1, longitude = 1 })
```
...
```
lua/
├── air-quality_sdk.lua    -- Main SDK module
...
The main module (`air-quality_sdk`) exports the SDK constructor
