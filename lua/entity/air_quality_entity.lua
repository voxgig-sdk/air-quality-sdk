...
local AirQualityEntity = {}
AirQualityEntity.__index = AirQualityEntity


function AirQualityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AirQualityEntity)
  self._name = "air_quality"
  self._client = client
...
function AirQualityEntity:get_name()
  return self._name
end


function AirQualityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AirQualityEntity.new(self._client, opts)
end
...
function AirQualityEntity:mark_deleted()
  self._deleted = true
end


function AirQualityEntity:deleted()
  return true == self._deleted
end


function AirQualityEntity:data_set(args)
  if args ~= nil then
...
function AirQualityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AirQualityEntity:match_set(args)
  if args ~= nil then
...
function AirQualityEntity:match_get()
...
function AirQualityEntity:stream(action, args, callopts)
...
function AirQualityEntity:_run_op(ctx, post_done)
  local utility = self._utility
