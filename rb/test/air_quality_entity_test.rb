...
class AirQualityEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AIRQUALITY_TEST_AIR_QUALITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    air_quality_ref01_data_dt0_loaded, err = air_quality_ref01_ent.load(air_quality_ref01_match_dt0, nil)
    assert_nil err
...
def air_quality_basic_setup(extra)
  ...
  entid_env_raw = ENV["AIRQUALITY_TEST_AIR_QUALITY_ENTID"]
