...
require_relative 'AirQuality_types'
...
class AirQualitySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AirQualityHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AirQualityHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AirQualityFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AirQualityUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AirQualityHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AirQualityHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AirQualityHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = AirQualityHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AirQualityError.new(
        "graphql_error", "AirQualitySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
