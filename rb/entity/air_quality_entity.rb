...
class AirQualityEntity
  ...
  def make
    opts = @_entopts.dup
    AirQualityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AirQualityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AirQuality, Hash] the current AirQuality data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AirQualityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AirQuality fields)
  def match_get
  ...
  #
  # @param reqmatch [AirQualityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.AirQuality.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [AirQuality, Hash] the loaded AirQuality; raises AirQualityError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AirQualityHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
