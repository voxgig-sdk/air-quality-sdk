...
func (sdk *AirQualitySDK) OptionsMap() map[string]any {
...
func (sdk *AirQualitySDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *AirQualitySDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *AirQualitySDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *AirQualitySDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *AirQualitySDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *AirQualitySDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("AirQualitySDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *AirQualitySDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *AirQualitySDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("AirQualitySDK: graphql: %s", msg)
		res["graphql"] = errors
	}
