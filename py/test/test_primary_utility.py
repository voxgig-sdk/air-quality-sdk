...
from airquality_sdk.core.spec import AirQualitySpec
from airquality_sdk.core.result import AirQualityResult
from airquality_sdk.core.response import AirQualityResponse
from airquality_sdk.core.operation import AirQualityOperation
from airquality_sdk.core.error import AirQualityError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return AirQualityError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = AirQualitySDK.test(None, None)
        ...
        class TestInitFeature(AirQualityBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(AirQualityBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = AirQualitySpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = AirQualitySDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = AirQualitySDK.test(None, setup_opts)
