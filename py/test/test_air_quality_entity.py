...
class TestAirQualityEntity:

    def test_should_create_instance(self):
        testsdk = AirQualitySDK.test(None, None)
        ent = testsdk.AirQuality(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _air_quality_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "air_quality." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AIR_QUALITY_TEST_AIR_QUALITY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        air_quality_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.air_quality")))
        air_quality_ref01_data = None
        if len(air_quality_ref01_data_raw) > 0:
...
def _air_quality_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/air_quality/AirQualityTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("AIR_QUALITY_TEST_AIR_QUALITY_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("AIR_QUALITY_TEST_LIVE") == "TRUE":
        ...
        client = AirQualitySDK(helpers.to_map(merged_opts))

    _live = env.get("AIR_QUALITY_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("AIR_QUALITY_TEST_EXPLAIN") == "TRUE",
        "live": _live,
