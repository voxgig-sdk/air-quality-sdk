...
from airquality_sdk.airquality_types import (
    AirQuality,
    AirQualityLoadMatch,
)
...
class AirQualityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AirQualityEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> AirQuality:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AirQuality().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
