# AirQuality SDK utility: make_request
...
module AirQualityUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AirQualityResponse.new({})
    ...
    if fetch_err
      ...
      response = AirQualityResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AirQualityResponse.new(fetched)
    else
