...
// AirQuality SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        AirQualitySDK $client,
        AirQualityUtility $utility
    ): AirQualityContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new AirQualityContext($ctxmap, null);
    ...
    private static function fixctx(AirQualityContext $ctx, AirQualitySDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?AirQualityError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AirQualityError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new AirQualityResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new AirQualityBaseFeature();
    ...
    {
        $hook_client = AirQualitySDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends AirQualityBaseFeature {
            public $hook_fn = null;
            public function TestHook(AirQualityContext $ctx): void
            {
    ...
    {
        $init_client = AirQualitySDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends AirQualityBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new AirQualitySDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new AirQualitySpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = AirQualitySDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = AirQualitySDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof AirQualityError) ? $result->err->msg : (string)$result->err;
