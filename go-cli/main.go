// Command AirQuality-cli is a CLI + REPL for the AirQuality SDK,
...
const prompt = "air-quality"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "air_quality"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AIR_QUALITY_APIKEY (api key), AIR_QUALITY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AIR_QUALITY_APIKEY carries the API key and
	// AIR_QUALITY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AIR_QUALITY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AIR_QUALITY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAirQualitySDK(opts)
