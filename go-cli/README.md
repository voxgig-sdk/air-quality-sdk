...
boru-driven command-line client **and** interactive REPL for the AirQuality
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/air-quality-cli)
make build

# 2. See usage (words, entities, env vars)
./air-quality-cli --help

# 3. Provide credentials once, via the environment
export AIR_QUALITY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./air-quality-cli load 1 air_quality            # {id:1} shorthand
./air-quality-cli load '{id:1}' air_quality       # explicit match map

# 5. Override the API base URL for a single call
AIR_QUALITY_BASE=https://api.example.com ./air-quality-cli load 1 air_quality

# 6. No arguments -> interactive REPL
./air-quality-cli
air-quality> load 1 air_quality
air-quality> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/air-quality-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/air-quality-cli load 1 air_quality
   ```
...
```sh
./air-quality-cli load 1 air_quality          # scalar shorthand for {id:1}
./air-quality-cli load '{id:1}' air_quality     # explicit match map
```
...
./air-quality-cli load 1 air_quality
```
...
Run with no arguments to open a REPL (prompt `air-quality>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `air_quality`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/air-quality-cli`. |
...
(`./air-quality-cli <expr>`) and interactively (the REPL), and expressions compose the
