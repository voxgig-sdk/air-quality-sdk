...
[MCP](https://modelcontextprotocol.io) server exposing the AirQuality SDK as
two agent tools — `air-quality_list` and `air-quality_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/air-quality-mcp)
make build

# 2. Provide credentials via the environment
export AIR_QUALITY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user air-quality \
  -- /absolute/path/to/air-quality-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./air-quality-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// air-quality_list: first page of records
{ "entity": "air_quality" }
{ "entity": "air_quality", "query": { } }

// air-quality_load: one record by id
{ "entity": "air_quality", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/air-quality-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user air-quality \
     -- "$PWD"/dist/*/air-quality-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `air-quality_list` and `air-quality_load` tools now appear
   in new sessions. Ask the agent to *"list air_quality using air-quality"*
   and it calls `air-quality_list` with `{"entity":"air_quality"}`.
...
```sh
export AIR_QUALITY_APIKEY=sk_live_xxx            # API key
export AIR_QUALITY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `air-quality_list` tool
...
### Call the `air-quality_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `air-quality_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `air-quality_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AIR_QUALITY_APIKEY` | API key sent with every request. |
| `AIR_QUALITY_BASE` | Optional override of the API base URL. |
...
```sh
./air-quality-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"air-quality_load","arguments":{"entity":"air_quality","query":{"id":1}}}}'
```
