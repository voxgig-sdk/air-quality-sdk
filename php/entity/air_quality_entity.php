...
// AirQuality SDK AirQuality entity
...
class AirQualityEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AirQualityEntity($this->_client, $opts);
    }

    /**
     * @param AirQuality|array $args AirQuality data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AirQualityHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return AirQuality|array The current AirQuality data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of AirQuality fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AirQualityHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of AirQuality fields).
     */
    ...
    /**
     * Load a single AirQuality.
     *
     * @param AirQualityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AirQualityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return AirQuality|array The loaded AirQuality as an assoc-array at the
     *   SDK boundary; throws AirQualityError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AirQualityHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
