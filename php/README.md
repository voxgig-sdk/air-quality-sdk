...
The PHP SDK for the AirQuality API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->AirQuality()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
$client = AirQualitySDK::test($testopts, $sdkopts);
```
...
| --- | --- | --- |
| `current` | `array` | Current conditions data |
| `current_units` | `array` | Units for current conditions variables |
...
| `hourly` | `array` | Hourly forecast data |
| `hourly_units` | `array` | Units for each hourly variable |
...
```
php/
├── airquality_sdk.php          -- Main SDK class
...
The main class (`airquality_sdk.php`) exports the SDK class
