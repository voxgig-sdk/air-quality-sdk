...
class AirQualitySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AirQualityHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AirQualityHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AirQualityFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AirQualityFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AirQualityUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AirQualityHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AirQualityHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AirQualityHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AirQualityError($op . "_allow",
                "AirQualitySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AirQualityHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AirQualityError("graphql_error",
                "AirQualitySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function AirQuality($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_air_quality === null) {
                $this->_air_quality = new AirQualityEntity($this, null);
            }
            return $this->_air_quality;
        }
        return new AirQualityEntity($this, $data);
    }
