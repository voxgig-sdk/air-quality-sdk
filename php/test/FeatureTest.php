...
class FtCtrl extends AirQualityControl
{
...
class FtClient
{
    ...
    private AirQualityUtility $utility;

    public function __construct(AirQualityUtility $utility, array $options)
    {
    ...
    public function get_utility(): AirQualityUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AirQualityContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AirQualityContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AirQualityContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AirQualityConfig::make_feature($name);
    ...
    private static function buildUrl(AirQualitySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AirQualityError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AirQualityResponse($fetched);
        }
    ...
    private function fail(AirQualityContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AirQualityContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
