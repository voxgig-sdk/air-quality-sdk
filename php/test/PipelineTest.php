...
{
    private static function utility(): AirQualityUtility
    {
        return new AirQualityUtility();
    }
    ...
    private static function ctx(array $over = []): AirQualityContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AirQualityContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AirQualityOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AirQualityError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AirQualityOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AirQualityMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AirQualityOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AirQualityMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AirQualityOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AirQualityMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AirQualitySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AirQualityMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new AirQualityResult([]),
        ]);
        [, $err] = AirQualityMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AirQualitySpec([]),
            'response' => null,
            'result' => new AirQualityResult([]),
        ]);
        [, $err] = AirQualityMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AirQualitySpec(['step' => 's']),
            'response' => new AirQualityResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AirQualityResult([]),
        ]);
        [, $err] = AirQualityMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AirQualitySpec(['step' => 's']),
            'response' => new AirQualityResponse(self::resp(200, ['v' => 1])),
            'result' => new AirQualityResult([]),
        ]);
        [, $err] = AirQualityMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AirQualitySpec(['step' => 's']),
            'response' => new AirQualityResponse(self::resp(200, ['v' => 2])),
            'result' => new AirQualityResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AirQualityMakeResponse::call($ctx);
    ...
    {
        $preset = new AirQualityResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AirQualitySpec([]),
            'response' => new AirQualityResponse([]),
            'result' => new AirQualityResult([]),
        ...
        [$got, $err] = AirQualityMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AirQualityResult([])]);
        [, $err] = AirQualityMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AirQualitySpec([]), 'result' => null]);
        [, $err] = AirQualityMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AirQualitySpec(['step' => 's']),
            'result' => new AirQualityResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AirQualityOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AirQualityMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AirQualitySpec(['step' => 's']),
            'result' => new AirQualityResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AirQualityOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AirQualityMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AirQualityResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AirQualitySpec([]),
            'result' => new AirQualityResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AirQualityMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AirQualityMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AirQualityError('boom', 'boom');
        $utility->fetcher = function (AirQualityContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AirQualitySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AirQualityMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AirQualityContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AirQualityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AirQualityResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AirQualityMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AirQualityContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AirQualitySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AirQualityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AirQualityResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AirQualitySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AirQualityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AirQualityMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AirQualityMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AirQualityResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AirQualityDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AirQualityResult(['ok' => false])]);
        $this->expectException(AirQualityError::class);
        AirQualityDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AirQualityResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AirQualityMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AirQualityMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AirQualityBaseFeature();
        $b = new AirQualityBaseFeature();
        AirQualityFeatureAdd::call($ctx, $a);
        AirQualityFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AirQualityBaseFeature
    {
        $f = new AirQualityBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AirQualityFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AirQualityContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AirQualitySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AirQualityPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AirQualityPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AirQualityResponse(['headers' => null]),
            'result' => new AirQualityResult([]),
        ]);
        AirQualityResultHeaders::call($ctx);
