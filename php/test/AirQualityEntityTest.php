...
class AirQualityEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AirQualitySDK::test(null, null);
        $ent = $testsdk->AirQuality(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = air_quality_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "air_quality." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set AIR_QUALITY_TEST_AIR_QUALITY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $air_quality_ref01_data = null;
        if (count($air_quality_ref01_data_raw) > 0) {
...
function air_quality_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/air_quality/AirQualityTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["air_quality01", "air_quality02", "air_quality03"] as $k) {
    ...
    $entid_env_raw = getenv("AIR_QUALITY_TEST_AIR_QUALITY_ENTID");
    ...
    if ($env["AIR_QUALITY_TEST_LIVE"] === "TRUE") {
        ...
        $client = new AirQualitySDK(Helpers::to_map($merged_opts));
    }
