...
class AirQualityMakeRequest
{
    public static function call(AirQualityContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new AirQualityResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new AirQualityResponse($fetched);
        } else {
