# AirQuality TypeScript SDK



The TypeScript SDK for the AirQuality API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.AirQuality()` — each with a small set of operations (`load`)
...
- Releases: [https://github.com/voxgig-sdk/air-quality-sdk/releases](https://github.com/voxgig-sdk/air-quality-sdk/releases)
...
```ts
const client = new AirQualitySDK({ apikey: '...' })
...
```ts
const entity = client.AirQuality()
...
| `tester(testopts?, sdkopts?)` | `AirQualitySDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AirQualitySDK.test(testopts?, sdkopts?)` | `AirQualitySDK` | Create a test-mode client. |
...
| `client` | `client(): AirQualitySDK` | Return the parent SDK client. |
...
Create an instance: `const air_quality = client.AirQuality()`
...
```ts
const air_quality = await client.AirQuality().load({ latitude: 1, longitude: 1 })
```
...
air-quality/
├── src/
│   ├── AirQualitySDK.ts        # Main SDK class
...
```ts
const airquality = client.AirQuality()
await airquality.load({ latitude: 1, longitude: 1 })
