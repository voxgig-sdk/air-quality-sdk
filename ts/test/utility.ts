...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.AIR_QUALITY_TEST_LIVE ||
    'TRUE' === process.env.AIR_QUALITY_TEST_OVERRIDE
  ) {
  ...
  m.AIR_QUALITY_TEST_EXPLAIN = process.env.AIR_QUALITY_TEST_EXPLAIN || m.AIR_QUALITY_TEST_EXPLAIN
