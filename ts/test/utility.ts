...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.AIRQUALITY_TEST_LIVE ||
    'TRUE' === process.env.AIRQUALITY_TEST_OVERRIDE
  ) {
  ...
  m.AIRQUALITY_TEST_EXPLAIN = process.env.AIRQUALITY_TEST_EXPLAIN || m.AIRQUALITY_TEST_EXPLAIN
