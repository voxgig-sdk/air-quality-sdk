...
//     (\`new AirQualitySDK(...)\` / \`AirQualitySDK.test(...)\` become
//     \`AirQualitySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"air_quality":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AirQualitySDK(...)\` and any
// \`AirQualitySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AirQualitySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AirQualitySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
